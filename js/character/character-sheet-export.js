...
class CharacterSheetExporter {
  ...
  async exportToWord(character) {
    ...
    try {
      ...
      children.push(new Paragraph({ text: licenseInfo.attributionText }));
      children.push(new Paragraph({ text: licenseInfo.productIdentityDisclaimer }));
      if (licenseInfo.srdUrl) {
        children.push(new Paragraph({ text: `${licenseInfo.referenceLabel}: ${licenseInfo.srdUrl}` }));
      }
  ...
  generateLicenseSectionHTML() {
    ...
    const attribution = this.escapeHtml(info.attributionText);
    ...
    const srdLink = srdUrl
      ? `<p style="margin: 0;">${this.escapeHtml(info.referenceLabel)}: <a href="${srdUrl}" target="_blank" rel="noopener noreferrer">Download from Wizards</a></p>`
      : '';
    return `
  ...
  // The license notices (text, SRD version label, reference link) come from site.js; there is no local copy of them
  getLicenseNotices() {
